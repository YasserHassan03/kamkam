...
/// All match results screen grouped by matchday
    ...
    return Scaffold(
      appBar: AppBar(
        title: tournamentAsync.maybeWhen(
          data: (t) => Text(t?.name ?? 'Results'),
          orElse: () => const Text('Results'),
        ),
      ),
          ...
          data: (matches) {
            if (matches.isEmpty) {
              return const EmptyStateWidget(
                ...
                subtitle: 'Results will appear once matches are played',
              );
            }

            // Filter to show only finished matches
            final results = matches.where((m) => 
            ...
            if (results.isEmpty) {
              return const EmptyStateWidget(
                ...
                subtitle: 'Come back after matches are completed',
              );
            }
